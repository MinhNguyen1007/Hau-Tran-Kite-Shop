...
export default async function AccountPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 md:py-14">
      ...
      <div className="mt-6 flex flex-col gap-5 rounded-xl border border-stone-200 bg-white p-5">
        <div className="flex items-center gap-3">
          <span className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-stone-100 text-stone-600">
          ...
          <div className="min-w-0">
            ...
            {profile.fullName && profile.email && (
              <p className="truncate text-sm text-stone-600">{profile.email}</p>
            )}
          </div>
        </div>
        ...
        {hasAdminAccess(profile.role) && (
          <Link
            href="/admin"
            className="self-start rounded-full bg-ink-950 px-5 py-2.5 text-sm font-bold text-white transition-colors hover:bg-ink-800 active:scale-[0.98]"
          >
            Vào trang quản trị
          </Link>
        )}
        ...
        <form action="/auth/dang-xuat" method="post" className="border-t border-stone-200 pt-5">
          <button
            type="submit"
      ...
      <Link
        href="/yeu-thich"
        className="mt-6 flex items-center gap-3 rounded-xl border border-stone-200 bg-white p-5 transition-colors hover:border-stone-400"
      >
      ...
      <p className="mt-4 rounded-xl border border-dashed border-stone-300 px-5 py-4 text-sm text-stone-600">
